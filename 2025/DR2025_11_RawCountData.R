...
surv_tows <- left_join(qu.tows, qu.surveys, by="SURVEY_SEQ")
# surv_tows <- rbind(data.frame(surv_tows, LIVECODE="L"), data.frame(surv_tows, LIVECODE="D"))
surv_tows_samp <- left_join(surv_tows, qu.hfreq, by="TOW_SEQ")
surv_tows_samp_hf <- left_join(surv_tows_samp, qu.heightfreq, by="HFREQ_SAMPLE_SEQ")
...
# And make the CSV... 
write.csv(surv_tows_samp_hf, "Y:/Offshore/Data requests/2025/DR2025_11_SPANS/DR2025_11_LE22.csv",row.names=F)
