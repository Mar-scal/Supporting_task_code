...
obs.trips <- read.table(paste0(direct, "data/Observed scallop trip metadata_", fleet, "_2025-08-08.csv",sep=""), sep=",",header=T,stringsAsFactors = F)
...
set_coord <- read.csv(paste0(direct, "data/Observed scallop trip metadata_tidysets_2025-08-08.csv"))
...
starfish_2425 <- discards_report[,c(1:7, which(names(discards_report) %in% c("ASTEROIDEA S.C.","PURPLE STARFISH","PURPLE SUNSTAR","STEGOPHIURA NODOSA","SUN STAR","BLOOD STAR")))]
...
paste0(direct, "data/discards_report_", species, "_", Sys.Date(), ".Rdata")

load("C:/Users/keyserf/Documents/temp_data/data/discards_report_all_2025-08-08.Rdata")
discards_report <- discards_report[,c(1:7, which(names(discards_report) %in% c("ASTEROIDEA S.C.","PURPLE STARFISH","PURPLE SUNSTAR","STEGOPHIURA NODOSA","SUN STAR","BLOOD STAR")))]
...
write.csv(x = monthly_eff, file = "C:/Users/keyserf/Documents/temp_data/alldiscards_seastars.csv")
...
ggplot() + geom_line(data=monthly, aes(ymd(paste0(year, "-", month, "-", 15)), discardrate)) + facet_wrap(~COMMON)+
  ylab("discard rate (kg/hm)")+
  xlab("month")

require(mgcv)
monthly$Time <- as.numeric(ymd(paste0(monthly$year, "-", monthly$month, "-15")))/1000
dr_gam <- gam(data=monthly[monthly$COMMON=="ASTEROIDEA S.C.",], discardrate ~ s(month, bs="cc") + s(Time))
summary(dr_gam)
plot(dr_gam)

dr_gam <- gam(data=monthly[monthly$COMMON=="SUN STAR",], discardrate ~ s(month, bs="cc") + s(Time))
summary(dr_gam)
plot(dr_gam)

dr_gam <- gam(data=monthly[monthly$COMMON=="STEGOPHIURA NODOSA",], discardrate ~ s(month, bs="cc") + s(Time))
summary(dr_gam)
plot(dr_gam)
...
ggplot() + geom_line(data=monthly, aes(ymd(paste0(year, "-", month, "-", 15)), totaldiscards)) + facet_wrap(~COMMON)+
