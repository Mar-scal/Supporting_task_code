...
projections<-function(model.out,C.p=c(200,300), my = max(model.out$data$NY),gy = max(model.out$data$NY))
{
  # Add the projected Catch from the start of survey year to the end of the calendar year
	...
	{
	  m <- as.data.frame(d$m[,my])
		m <- apply(m, 1, median, na.rm=T)
		mR <- as.data.frame(d$mR[,my])
		mR <- apply(mR, 1, median, na.rm=T)
		g <- median(d$g[gy])
		gR <- median(d$gR[gy])
		print(data.frame(m=median(m), mR=median(mR), g=median(g), gR=median(gR)))
	  
	  # year 1 projection.  First the process equation.
		Pmed.p[[i]]<- log(exp(-m)*(g)*(d$P[,d$NY]-C.p[i]/d$K)+exp(-mR)*(gR)*d$r[,d$NY])
		...
		Pmed.p2[[i]]<-log(exp(-d$m[,my])*(d$g[gy])*P.p[[i]]+exp(-d$mR[,my])*(d$gR[gy])*d$r[,d$NY])
		# Get P
