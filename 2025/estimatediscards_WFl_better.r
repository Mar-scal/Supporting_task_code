...
obs.trips <- read.table(paste0(direct, "data/Observed scallop trip metadata_", fleet, "_2025-07-28.csv",sep=""), sep=",",header=T,stringsAsFactors = F)
...
table(hooks$PNTCD_ID)[[1]]==table(hooks$PNTCD_ID)[[2]]
...
set_coord <- read.csv(paste0(direct, "data/Observed scallop trip metadata_tidysets_2025-07-28.csv"))

set_coord$area[set_coord$area %in% c("Sab", "Mid")] <- "SabMid"
...
hooks_area_trip_na <- rbind(hooks_area_trip_na[!hooks_area_trip_na$TRIP == splits,], splits_na)
...
discards_sets[discards_sets$tripset %in% discards_sets_na$tripset,]$area <- discards_sets_na$bank
...
for(i in discards_effort_2_lost){
  ...
  if(length(unique(temp$hm[!is.na(temp$hm)])) == 1) {
    ...
    temp$COMMON <- temp[!is.na(temp$hm),]$COMMON
...
if("vesid" %in% names(discards_effort)) discards_report <- dplyr::select(discards_effort, VESSEL_NAME, bank, vesid, vrnum, TRIP, obshooks, totalhooks, prophooks, COMMON, totaldiscards, hm)
if(!"vesid" %in% names(discards_effort)) discards_report <- dplyr::select(discards_effort, VESSEL_NAME, bank, vrnum, TRIP, obshooks, totalhooks, prophooks, COMMON, totaldiscards, hm)
...
discards_report <- discards_report %>% 
  group_by(VESSEL_NAME, vrnum, TRIP, COMMON) %>%
  summarize(
    obshooks=sum(obshooks),
...
discards_report2 <- arrange(
  dcast(data=discards_report, VESSEL_NAME + vrnum + TRIP + obshooks + totalhooks + prophooks + bank + hm ~ COMMON, sum, value.var="discards"),
  TRIP)
...
discards_report <- arrange(dplyr::select(discards_report, VESSEL_NAME, vrnum, bank, TRIP, date.sailed, date.land, obshooks, totalhooks,
                                  prophooks, COMMON, discards, hm), 
                           date.land)
...
monthly <- discards_report %>%
  group_by(year, month) %>%
  summarize(monthlydiscards = sum(discards/prophooks, na.rm=T),
            monthlyobseffort = sum(hm, na.rm=T))
...
monthly <- left_join(monthly, fleeteffort)
monthly$discardrate <- monthly$monthlydiscards / monthly$monthlyobseffort
monthly$totaldiscards_kg <- monthly$totaleffort*monthly$discardrate
monthly %>% View()
