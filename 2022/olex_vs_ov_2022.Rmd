...
```
Raw OV matches smoothed OV very closely.     
Smoothed Olex is much shorter than raw Olex. Raw Olex is longer than raw OV.    
...
Look at the distances using raw values too. Summary is raw Olex length - raw OV length.     
```{r, echo=F}
tracks <- tracks %>%
  st_transform(32620) %>%
...
ggplot() + geom_point(data=rawlengthcomp, aes(as.numeric(length_olex), as.numeric(length))) +
  theme_bw() + theme(panel.grid=element_blank()) + xlab("olex length (m)") + ylab("OV length (m)") +
  geom_abline(aes(slope=1, intercept=0)) + ggtitle("Raw (before smoothing)")
```  

Raw Olex tows are longer in distance than raw OV tows.     
The difference is mitigated by smoothing, but not completely.    
Using a smoothing (moving average) window of 14 seems to mitigate bias.    
