...
plot_grid(out[[1]]$impact_HCR1$evaluation_catch + theme(legend.position = "topright") + ylab("Catch potential (mt)"), 
...
plot_grid(out[[1]]$impact_HCR2$evaluation_catch + theme(legend.position = "topright")+ ylab("Catch potential (mt)"),  
...
med.pred <- pred.eval.dat %>%
  ...
  dplyr::mutate(med_real = med2-med0,
                med_1 = med2-med1,
                med_greater_than_real = ifelse(med2 > med0, 1, 0),
                med_greater_than_1 = ifelse(med2 > med1, 1, 0))

pred.eval.summary <- full_join(range.pred, med.pred, by=c("year", "surplus"))

scenario.pred <- full_join(range.pred, med.pred, by=c("year", "surplus")) %>%
  dplyr::filter(!is.na(range2) & !is.na(range0)) %>%
  dplyr::group_by(surplus) %>%
  dplyr::summarise(prop_range_greater_real = (sum(range_greater_than_real)/n()),
                   prop_range_greater_1 = (sum(range_greater_than_1)/n()),
                   prop_med_greater_real = (sum(med_greater_than_real)/n()),
                   prop_med_greater_1 = (sum(med_greater_than_1)/n()))
