proj_eval_plot <- function(object, area, surplus, mu, ref.pts, save){
  ...
  if(save==T){
    ...
    png(filename = paste0("./Offshore/", area, "/", tag1, "/pred_eval_F", tag, ".png"), width=11, height=8.5, units="in", res=400)
    print(pred.eval.F)
    dev.off()
    
    png(filename = paste0("./Offshore/", area, "/", tag1, "/zoom_pred_eval_F", tag, ".png"), width=11, height=8.5, units="in", res=400)
