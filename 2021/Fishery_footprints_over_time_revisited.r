...
yr <- 2021
years.t = 2011:2021 # The years of interest make temporary as this could be overwritten when I bring in the survey results...
...
logs_and_fish(loc="offshore",year = 2011:yr,direct=direct)
...
spatial_history <- function(bnk, years, high=25, subarea=NULL) {
  yr <- max(years)
  ...
  n.high <- arrange(full_join(aggregate(catch~ year,high.catch,length), data.frame(year=years)), year)
  ft.n.high <- arrange(full_join(aggregate(catch~ year,high.ft.catch,length), data.frame(year=years)), year)
  wf.n.high <- arrange(full_join(aggregate(catch~ year,high.wf.catch,length), data.frame(year=years)), year)
  ...
  a <- ggplot() + geom_point(data=annual.summary, aes(year, cells_fished)) + 
    geom_line(data=annual.summary, aes(year, cells_fished)) + 
    theme_bw() +
    scale_x_continuous(breaks=min(years):max(years))
  b <- ggplot() + geom_point(data=annual.summary, aes(year, num_high)) + 
    geom_line(data=annual.summary, aes(year, num_high)) + theme_bw() + 
    scale_x_continuous(breaks=min(years):max(years))
  c <- ggplot() + geom_point(data=annual.summary, aes(year, prop_high)) + 
    geom_line(data=annual.summary, aes(year, prop_high)) + theme_bw() +
    scale_x_continuous(breaks=min(years):max(years))
  d <- ggplot() + geom_point(data=annual.summary, aes(year, median_catch)) + 
    geom_line(data=annual.summary, aes(year, median_catch)) + theme_bw() + 
    scale_x_continuous(breaks=min(years):max(years))
  e <- ggplot() + geom_point(data=annual.summary, aes(year, median_effort)) + 
    geom_line(data=annual.summary, aes(year, median_effort)) + theme_bw() + 
    scale_x_continuous(breaks=min(years):max(years))
  
  require(patchwork)
  print(a/b/c/d/e + plot_annotation(
  ...
  medcatch <- left_join(pivot_longer(annual.summary[,c("year", "median_catch", "median_effort", "cells_fished", "prop_high", "num_high")], 
            ...
            annual.summary[,c("year", "median_catch")])
  ...
  medeff <- left_join(pivot_longer(annual.summary[,c("year", "median_catch", "median_effort", "cells_fished", "prop_high", "num_high")], 
                                     c(median_catch, median_effort, cells_fished, prop_high, num_high)),
                        annual.summary[,c("year", "median_effort")])
  ...
  cellsf <- left_join(pivot_longer(annual.summary[,c("year", "median_catch", "median_effort", "cells_fished", "prop_high", "num_high")], 
                                   c(median_catch, median_effort, cells_fished, prop_high, num_high)),
                      annual.summary[,c("year", "cells_fished")])
  ...
  proph <- left_join(pivot_longer(annual.summary[,c("year", "median_catch", "median_effort", "cells_fished", "prop_high", "num_high")], 
                                   c(median_catch, median_effort, cells_fished, prop_high, num_high)),
                      annual.summary[,c("year", "prop_high")])
  ...
  numh <- left_join(pivot_longer(annual.summary[,c("year", "median_catch", "median_effort", "cells_fished", "prop_high", "num_high")], 
                                  c(median_catch, median_effort, cells_fished, prop_high, num_high)),
                     annual.summary[,c("year", "num_high")])
  ...
  print((f|g|h|i|j) + plot_annotation(
...
spatial_history(years=2011:2021, bnk="GBa", high=100)
