...
windows(11,11)
p <-pecjector(area = "inshore",direct_fns = "D:/Github/Offshore/Assessment_fns/DK/",repo = "D:/Github/Offshore/GIS_layers/DK/",
...
labels <- read.csv("D:/R/Data/Maps/approved/Fishing_Area_Borders/scallop_labels.csv")
names(labels)
labels <- st_as_sf(labels,coords = c("X","Y"),crs = 4326)
#labels <- labels %>% dplyr::filter(region == 'inshore_detailed_survey')
# Moved into the 'labels' folder, the inshore strata and labels still need cleaned up!
st_write(obj = labels, dsn ="D:/Github/Offshore/GIS_layers/DK/other_boundaries/labels", layer = 'labels.shp',driver="ESRI Shapefile")

p2 <- p + geom_sf_text(data=labels,aes(label = lab_short),angle=35)
p2

repo <- "D:/Github/Offshore/GIS_layers/DK/"
