...
# FK's attempt to screen manually 
# # screen following the methods of https://waves-vagues.dfo-mpo.gc.ca/library-bibliotheque/40885690.pdf (page)
# nafo <- github_spatial_import(subfolder = "NAFO", zipname = "Subareas.zip")
# 
# fish_sf <- fish_sf[fish_sf$sfa %in% c("25A.shp"),]
# 
# level2 <- nafo$level_2[grep(x = nafo$level_2, pattern="4")]
# level2 <- level2[nchar(level2)>2]
# 
# ggplot() + geom_sf(data=nafo[nafo$level_2 %in% level2,], aes(fill=level_2), alpha=0.2) +
#   geom_sf_label(data=nafo[nafo$level_2 %in% level2,], aes(label=level_2))#subareas
# 
# nafo <- nafo[nafo$level_2 %in% level2,]
# 
# fish_sf_nafo <-  st_intersection(fish_sf, nafo)
# 
# # calculating footprints
# crs <- 32620
# foot <- st_as_sf(fish_sf_nafo[grep(x=fish_sf_nafo$sfa, pattern="25A"),]) %>%
#   st_transform(crs)
# base <- offshore[grep(x=offshore$ID, pattern="25A"),] %>% st_transform(crs)
# 
# #create grid for footprints
# r <- create_grid(gridsize = sqrt(50)*1000, polygon = base)
# 
# # apply grid without screening:
# foot_grid <- foot %>%
#   st_intersection(r) %>%
#   group_by(sfa, cell, year, level_2) %>%
#   dplyr::summarize(kg = sum(pro.repwt, na.rm = T),
#                    hm = sum(hm, na.rm=T))
# 
# st_geometry(foot_grid) <- NULL
# 
# foot_grid <- dplyr::left_join(r, foot_grid)
# 
# # summarize for comparison
# foot_grid %>% dplyr::group_by(year) %>%
#   dplyr::summarize(kg=sum(kg),
#                    hm=sum(hm)) %>% 
#   st_drop_geometry()
# 
# # screening per open data paper
# screen <- foot %>%
#   group_by(sfa, year, level_2) %>%
#    dplyr::summarize(nvessels = length(unique(vrnum)) + length(unique(vesid))) %>%
#     filter(nvessels>4)
# 
# #ggplot() + geom_sf(data=screen)
# st_geometry(screen) <- NULL
# 
# foot_screen <- left_join(screen, foot)
# foot_screen <- st_as_sf(foot_screen) %>%
#   st_transform(crs)
# 
...
#                    hm = sum(hm, na.rm=T))
# 
# st_geometry(foot_screen) <- NULL
# 
# foot_screen <- dplyr::left_join(r, foot_screen)
# 
# # compare screening to unscreened
# screened <- foot_screen %>% dplyr::group_by(year) %>%
#   dplyr::summarize(screen_kg=sum(kg),
#                    screen_hm=sum(hm)) %>% 
#   st_drop_geometry()  
# 
# unscreened <- foot_grid %>% dplyr::group_by(year) %>%
#   dplyr::summarize(kg=sum(kg),
#                    hm=sum(hm)) %>% 
#   st_drop_geometry()
# 
...
# 
# # plot limits
# lims <- foot_screen[!is.na(foot_screen$kg),] %>%
#   st_combine() %>%
#   st_convex_hull() %>%
...
# 
# png(filename=paste0("Y:/Offshore/Data requests/2024/Maps_for_SPANS_CDDrequest/footprint_kg_25A.png"),width=16, height=10, units = "in", res=420)
# print(bp +
#         geom_sf(data=foot_grid[!is.na(foot_grid$kg) & !is.na(foot_grid$year),], fill="transparent", colour=NA, show.legend=T) +
#         geom_sf(data=foot_screen[!is.na(foot_screen$kg) & !is.na(foot_screen$year),], aes(fill=kg/1000), colour=NA, show.legend=T) +
...
# 
# # for CSV
# foot_screen <- foot_screen[!is.na(foot_screen$kg),]
# foot_screen$sfa <- gsub(x = foot_screen$sfa, ".shp", "", fixed=T)
# foot_screen <- st_centroid(foot_screen)
# foot_screen <- st_transform(foot_screen, 4326)
# coords <- as.data.frame(st_coordinates(foot_screen))
# foot_screen$lon <- coords$X
# foot_screen$lat <- coords$Y
# st_geometry(foot_screen) <- NULL
# names(foot_screen) <- c("ID", "cell", "SFA", "year", "NAFO", "landings_kg", "effort_hm", "lon_centroid", "lat_centroid")
# foot_screen <- arrange(foot_screen, year, ID, cell)
# foot_screen$lon_centroid <- round(foot_screen$lon_centroid, 5)
# foot_screen$lat_centroid <- round(foot_screen$lat_centroid, 5)
# foot_screen <- dplyr::select(foot_screen, SFA, year, lon_centroid, lat_centroid, NAFO, landings_kg, effort_hm)
# write.csv(x=foot_screen, file = "Y:/Offshore/Data requests/2024/Maps_for_SPANS_CDDrequest/logdata_1990-2010_SabMid_gridded_screened.csv")
# 
# 
# ggplot() + geom_sf(data=nafo) + geom_point(data=foot_screen, aes(lon_centroid, lat_centroid, colour=NAFO)) +
#   xlim(-65, -55) +
#   ylim(42, 46)
...
fish_sf <- dplyr::select(fish_sf, year, lon, lat, pro.repwt, hm, vesid, vrnum, licence)
...
#log_mike$SFA <- "25A"
log_mike <- dplyr::select(log_mike, year, kgs, lon, lat)#SFA, 
names(log_mike) <- c("year", "landings_kg", "lon_centroid", "lat_centroid")#"SFA", 
log_mike <- arrange(log_mike, year, lon_centroid, lat_centroid)#SFA, 
...
#log_mike_hm$SFA <- "25A"
log_mike_hm <- dplyr::select(log_mike_hm, year, hm, lon, lat)#SFA, 
names(log_mike_hm) <- c("year", "effort_hm", "lon_centroid", "lat_centroid")#"SFA", 
log_mike_hm <- arrange(log_mike_hm, year, lon_centroid, lat_centroid)#SFA, 
...
#cdd_mike$SFA <- "25A"
cdd_mike <- dplyr::select(cdd_mike,YEAR, kgs, lon, lat)# SFA, 
names(cdd_mike) <- c("year", "landings_kg", "lon_centroid", "lat_centroid")#"SFA", 
cdd_mike <- arrange(cdd_mike, year, lon_centroid, lat_centroid)#SFA, 
