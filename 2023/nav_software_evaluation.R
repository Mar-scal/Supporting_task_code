...
levels(lengthcomp_long$name2) <- c("Olex (brut)", "Olex (corrigé)", "Oceanvision (méthode courant)")
...
ggplot() + geom_point(data=lengthcomp_ov, aes(as.numeric(length_ovd), as.numeric(value), colour=name), alpha=0.5) +
  ...
  geom_text_repel(data=labs, aes(x, y, label=lab), box.padding=3, min.segment.length = 1, arrow = arrow(length = unit(0.2, units = "in")))+
  theme_bw() + 
...
labs <- data.frame(x=925, y=c(925, 962), lab=c("Olex (corrigé)", "Olex (brut)"))
png("Y:/Offshore/Assessment/2023/Supporting_tasks/Olex_transition_linear_fr.png", width=6, height=4, res=400, units="in")
ggplot() + geom_point(data=lengthcomp_ov, aes(as.numeric(length_ovd), as.numeric(value), colour=name), alpha=0.5) +
  geom_smooth(data=lengthcomp_ov, aes(as.numeric(length_ovd), as.numeric(value), colour=name), method="lm") +
  geom_abline(slope=1, intercept=0, linetype="dashed") +
  geom_text_repel(data=labs, aes(x, y, label=lab), box.padding=3, min.segment.length = 1, arrow = arrow(length = unit(0.2, units = "in")))+
