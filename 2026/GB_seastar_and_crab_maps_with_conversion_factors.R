...
# temp <- tempfile()
# download.file("https://raw.githubusercontent.com/Dave-Keith/GIS_layers/master/other_boundaries/other_boundaries.zip", temp, quiet=T)
...
nv.missions <- c("CAR2025010", "CAR2025002","CAR2024010","CAB2024003","CAR2023011","CAR2023002",
                 "CAB2022010", "CAR2022102","CAR2021241","CAR2021240","CAB2026002")
...
#Mar.utils::get_data_tables(schema="GROUNDFISH",cxn=cxn,tables="GSMISSIONS",force.extract=T)
...
n.stations <- gb.all.unique.tows |> dplyr::filter(type %in% c(1,5)) |> group_by(season) |>  distinct(year, setno) |>  count(year)
spring.t1.5 <- n.stations[n.stations$season == "SPRING",]
summary(spring.t1.5$n)
...
ss.by.spec <- all.tmp |> collapse::fgroup_by(spec) |> collapse::fsummarise(num = sum(totno,na.rm=T),
                                                                               bm = sum(totwgt,na.rm=T))
...
crab.by.spec <- all.crabs |> collapse::fgroup_by(spec) |> collapse::fsummarise(num = sum(totno,na.rm=T),
                                                                               bm = sum(totwgt,na.rm=T))
...
gb.crab.all.2 <- gb.crab.all[,-which(names(gb.crab.all) == "FID"),]
...
# now sea stars
# Clean up the Infs that show up because weights weren't taken on some tows and/or for some species.
gb.ss.all.2 <- gb.ss.all[,-which(names(gb.ss.all) == "FID"),]
